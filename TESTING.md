...
- rich synthetic DDS metadata plus managed HKX/HKT container, SDK, and tag-section metadata; an explicit metadata-only result with no renderer artifact or HKX-specific renderer handshake; and model-package preparation without a grid, gizmo, texture, or source mutation;
