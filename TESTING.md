...
- canonical Item Finder catalog rows from the native indexer, cache-schema and search-index reconstruction, model-variant grouping, secondary-ID and localized-name search, category/material facets, bounded 72-row paging, exact-versus-related Archive Browser entry scopes, persistent settings, automatic background preload wiring, foreground-work priority, 64-item worker batches, a 96-image UI LRU, and 120-pixel DirectXTex thumbnail cold/warm behavior that proves a warm hit needs neither its deleted DDS staging input nor another converter launch;
...
- rich synthetic DDS metadata plus managed HKX/HKT container, SDK, and tag-section metadata; an explicit metadata-only result with no renderer artifact or HKX-specific renderer handshake; and model-package preparation without a grid, gizmo, texture, or source mutation. Archive Lite's simple renderer source contract also requires matte geometry shading, reduced rim response, a one-pixel dark topology overlay, and an untextured solid-wire mode that reaches the D3D11 wire pass;
