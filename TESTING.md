...
This gate configures and builds `cdmw_archive_core` and `cdmw_preview_core`, runs their native self-tests, builds the .NET 10 solution and .NET/Vortice renderer, and runs the managed console test runner. Covered behavior includes:
...
- native model-package adaptation, exact geometry-length checks, path-containment rejection, and a headless synthetic package load through the real .NET renderer;
...
In addition to rerunning the focused checks in Release, this gate publishes the app, worker, and .NET/Vortice renderer self-contained for `win-x64`, includes both native read-only services, writes a SHA-256 package-content manifest, and calls:
