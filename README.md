...
- Read-only HKX/HKT visual preparation through the native Rust `cd-hkx.exe` parser. Skeleton-bearing tagfiles are displayed as a texture-free joint-and-bone preview; files without a renderable skeleton receive a bounded object-structure preview when their native object graph can be decoded. The parser is isolated, cancellable, output-bounded, and never mutates the archive source.
...
```text
CDMW-Archive-Lite-0.6.0-Standalone-win-x64.exe (single-file Native AOT launcher)
          |
...
CdmwArchiveLite.Worker.exe (.NET 10, cancellable operations)
          ...
          +-- cd-hkx.exe (Rust, bounded HKX skeleton/object-structure parser)
