...
- Optional known in-game names and a searchable **Item Finder** built from the archive's ItemInfo/localization tables. The shared native catalog supplies the same raw item rows to full CDMW and Lite; Lite preserves exact localized names separately from related-name hints, groups model variants, filters by category/material evidence, and pages 72 icon tiles at a time. **Exact links** and **Related set** scope the normal Archive Browser, so its existing sorting and export paths remain authoritative. Item icons are prepared as persistent 120-pixel DirectXTex thumbnails in the background, visible rows take priority, warm hits perform no archive decode or converter launch, and the WPF image cache is capped at 96 entries. Query, filters, and dialog size persist in Lite settings.
